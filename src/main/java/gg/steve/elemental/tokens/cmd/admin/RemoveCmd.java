...
public class RemoveCmd {
    ...
    public static void remove(CommandSender sender, String[] args) {
        // /eta remove nbdsteve token 10
        if (!PermissionNode.REMOVE.hasPermission(sender)) {
            CommandDebug.INSUFFICIENT_PERMISSION.message(sender, PermissionNode.REMOVE.get());
            return;
        }
        if (args.length != 4) {
        ...
        PlayerTokenManager.removeTokens(type, target, amount);
